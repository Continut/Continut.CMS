...
;(function ( $, window, undefined ) {
    ...
    // ramo : local modification
    Pep.prototype.reAttach = function() {
        if (this.options.prevBrother.length == 1) {
            this.options.prevBrother.after(this.$el);
        } else {
            if (this.options.prevParent.length == 1) {
                this.options.prevParent.prepend(this.$el);
            }
        }
    };
