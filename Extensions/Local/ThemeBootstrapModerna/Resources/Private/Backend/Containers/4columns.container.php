<div class="row">
	<div class="col-lg-3">
		<div class="panel panel-warning">
			<div class="panel-heading">
				<div class="panel-title"><?= $this->__("backend.themeBootstrapModerna.layout.container.column1") ?></div>
			</div>
			<div class="panel-body">
				<?= $this->showContainerColumn(4); ?>
			</div>
		</div>
	</div>
	<div class="col-lg-3">
		<div class="panel panel-warning">
			<div class="panel-heading">
				<div class="panel-title"><?= $this->__("backend.themeBootstrapModerna.layout.container.column2") ?></div>
			</div>
			<div class="panel-body">
				<?= $this->showContainerColumn(5); ?>
			</div>
		</div>
	</div>
	<div class="col-lg-3">
		<div class="panel panel-warning">
			<div class="panel-heading">
				<div class="panel-title"><?= $this->__("backend.themeBootstrapModerna.layout.container.column3") ?></div>
			</div>
			<div class="panel-body">
				<?= $this->showContainerColumn(6); ?>
			</div>
		</div>
	</div>
	<div class="col-lg-3">
		<div class="panel panel-warning">
			<div class="panel-heading">
				<div class="panel-title"><?= $this->__("backend.themeBootstrapModerna.layout.container.column4") ?></div>
			</div>
			<div class="panel-body">
				<?= $this->showContainerColumn(7); ?>
			</div>
		</div>
	</div>
</div>